...
export class ProfilePageComponent implements OnInit {
  ...
  @Input() userProfile: any = null; // Profilo dell'utente (può contenere anche l'immagine)
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      this.isLoading = false;
      this.routineSub = interval(1805000).subscribe(() => {
  ...
  async manageVisitor() {
    if (this.username && !this.navigationRedirected) {
      ...
      try {
        const profile = await firstValueFrom(this.userService.getUserByUsername(this.username));
        ...
        this.userProfile = {
          ...
          profileImage: profile.profileImage || 'assets/default-profile.png',
          bio: profile.bio || '',
          bioLink: profile.bioLink || 'www.example.com',
          followersCount: this.followerList.length, 
          cocktails: profile.cocktails || [],
  ...
  async manageFollow(userName: string) {
    ...
    try {
      ...
      if (userName) {
        this.userService.getUserByUsername(userName).subscribe(
          profile => {
            this.userService.getFollowers(this.userProfile.id).then(
              followers => {
                this.followerList = followers;
                this.userProfile = {
                  coverImage: profile.coverImage || 'assets/default-profile-background.png',
